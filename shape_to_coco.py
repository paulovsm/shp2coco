    ...
    if os.path.exists(ROOT_DIR):
        ...
        for root, _, files in os.walk(IMAGE_DIR):
            ...
            for image_filename in image_files:
                ...
                tif = TIFF.open(image_filename, mode='r')
                image = tif.read_image()
                image_info = pycococreatortools.create_image_info(
                    image_id, os.path.basename(image_filename), (image.shape[0], image.shape[1]))
                ...
                for root, _, files in os.walk(ANNOTATION_DIR):
                    ...
                    for annotation_filename in annotation_files:
                        ...
                        #binary_mask_tif = TIFF.open(annotation_filename, mode='r')
                        #binary_mask = np.where(binary_mask_tif.read_image() > 0, 1, 0).astype(np.uint8)
                        binary_mask = np.asarray(Image.open(annotation_filename)
                                                 .convert('1')).astype(np.uint8)

                        annotation_info = pycococreatortools.create_annotation_info(
                            segmentation_id, image_id, category_info, binary_mask,
                            (image.shape[0], image.shape[1]), tolerance=2)
        ...
        with open('{}/instances_deforestation_{}2022.json'.format(ROOT_DIR, MARK), 'w') as output_json_file:
...
def main():
    ...
    from_mask_to_coco(ROOT_DIR, 'train', "deforestation_2022", "annotations")
    from_mask_to_coco(ROOT_DIR, 'eval', "deforestation_2022", "annotations")
    from_mask_to_coco(ROOT_DIR, 'test', "deforestation_2022", "annotations")
